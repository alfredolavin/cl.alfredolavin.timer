#!/usr/bin/env python3
"""Generate contents/ui/code/icons.js from an installed Nerd Font.
...
def main():
    ...
    for cat, words in CATEGORIES:
        for w in words.split():
            for cand in ("md-" + w, "md-" + w + "_outline", "fa-" + w):
                cp = by_name.get(cand)
                if cp and cp not in seen:
                    seen.add(cp)
                    featured.append([format(cp, "x"), cand, cat])
                    break
    for name in FA_EXTRA:
        cp = by_name.get(name)
        if cp and cp not in seen:
            seen.add(cp)
            featured.append([format(cp, "x"), name, "Font Awesome"])

    everything = sorted(by_name.items(), key=lambda kv: kv[1])
    with open(OUT, "w") as f:
        f.write("// Generated by tools/gen_icons.py from %s. Do not edit.\n.pragma library\n\n" % os.path.basename(font))
        f.write("var categories = %s;\n\n" % json.dumps(["All featured"] + [c for c, _ in CATEGORIES] + ["Font Awesome", "Everything"]))
        f.write("// [codepoint hex, glyph name, category]\nvar featured = [\n")
        f.write(",\n".join(json.dumps(x) for x in featured))
        f.write("\n];\n\n// Every PUA glyph in the font: [codepoint hex, glyph name]\nvar all = [")
        f.write(",".join(json.dumps([format(cp, "x"), n]) for n, cp in everything))
        f.write("];\n")
    print("font:", font, "featured:", len(featured), "all:", len(everything))
