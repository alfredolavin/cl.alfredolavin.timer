#!/usr/bin/env python3
"""Generate contents/ui/code/icons.js (icon list plus SVG outlines) from a Nerd Font.

The widget ships the outlines, so no font needs to be installed to use it.
...
def svg_of(glyphs, name):
    """[viewBox, path] of the glyph centered in a square (font units, y flipped), or None if empty."""
    bp = BoundsPen(glyphs)
    glyphs[name].draw(bp)
    if not bp.bounds:
        return None
    ...
    return ["%d %d %d %d" % (vx, vy, round(side), round(side)), pen.getCommands()]
...
def main():
    ...
    with open(OUT, "w") as f:
        ...
        f.write("// [codepoint hex, glyph name, category]\nvar icons = [\n")
        f.write(",\n".join(json.dumps(x) for x in chosen))
        f.write("\n];\n\n// codepoint hex -> [SVG viewBox, SVG path]\nvar svg = {\n")
        f.write(",\n".join("%s: %s" % (json.dumps(h), json.dumps(svg_of(glyphs, n))) for h, n, _ in chosen))
        f.write("\n};\n\n")
        f.write("// SVG data URL of an icon filled with a QML color\n"
                "function url(hex, color) {\n"
                "    var s = svg[hex];\n"
                "    if (!s)\n"
                "        return \"\";\n"
                "    var fill = \"rgb(\" + Math.round(color.r * 255) + \",\" + Math.round(color.g * 255) + \",\" + Math.round(color.b * 255) + \")\";\n"
                "    return \"data:image/svg+xml;utf8,\" + encodeURIComponent('<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"' + s[0]\n"
                "        + '\"><path fill=\"' + fill + '\" fill-opacity=\"' + color.a + '\" d=\"' + s[1] + '\"/></svg>');\n"
                "}\n")
